...
/// <summary>
/// Generic implementation of a Pn-Counter (Positive-Negative Counter).
/// Allows managing quantities (Stocks, Balance) without conflicts.
/// </summary>
public class DistributedCounter
{
    // P : Increments (Inputs) per Node
    [JsonInclude]
    public Dictionary<string, int> P { get; set; } = new();

    // N : Decrements (Outputs) per Node
    [JsonInclude]
    public Dictionary<string, int> N { get; set; } = new();

    // Calculated Value : Sum(P) - Sum(N)
    [JsonIgnore]
    ...
    /// <summary>
    /// Mathematical merge function (Commutative, Associative, Idempotent).
